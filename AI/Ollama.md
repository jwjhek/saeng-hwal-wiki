...
**Ollama**는 오픈 웨이트 LLM을 **이 컴퓨터(또는 공식 클라우드)** 에서 돌리는 런타임이다. ChatGPT 같은 **챗 구독 제품이 아니고**, Llama·[[젬마]]·Qwen 같은 **모델 파일도 아니다.** 모델을 받아 대화하고, 로컬 [[API]]로 앱에 붙이는 쪽에 가깝다.
...
관련: [[현존 AI 비교]] · [[파라미터 수]] · [[젬마]] · [[RAG]] · [[OpenAI Platform]] · [[API]] · [[VS Code 추천 확장]]
...
모델 이름은 라이브러리가 수시로 바뀐다. 퀵스타트 예(`gemma4` 등)를 이 글에 고정하지 않는다. [검색](https://ollama.com/search)에서 **파라미터 수·용량·라이선스**를 본다. `7B`·`26B` 읽는 법은 [[파라미터 수]]. 젬마 가문은 [[젬마]].
...
공식 이미지 `ollama/ollama` → [[Docker 사용법]]. GPU 통과는 드라이버·런타임이 맞아야 한다. 이 PC VRAM에 모델이 안 들어가면 클라우드나 전용 박스([[DGX Spark]])를 본다.
