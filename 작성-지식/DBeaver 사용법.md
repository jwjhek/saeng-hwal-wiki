...
[[JDK]]: 최근 배포본은 런타임을 포함하는 경우가 많다. 개발용 JDK와 달라도 된다. 
...
Windows·Linux 기본값. macOS는 **Cmd**가 **Ctrl** 자리인 경우가 많다. 
안 먹으면 `Window` → `Preferences` → `General` → **Keys**에서 검색·재지정.

공식: [Shortcuts](https://dbeaver.com/docs/dbeaver/Shortcuts/)

### 5.1 SQL 편집·포맷

| 단축키 | 동작 |
|--------|------|
| **`Ctrl` + `Shift` + `F`** | **SQL 포맷** — 선택 구간 들여쓰기·정렬. 복붙한 쿼리 정리할 때 유용 |
| `Ctrl` + `/` | 한 줄 **주석** 토글 |
| `Ctrl` + `Shift` + `/` | **블록 주석** 토글 |
| `Ctrl` + `Shift` + `Space` | **자동 완성** 제안 |
| `Ctrl` + `Shift` + `]` | 괄호 **사이** 텍스트 선택 |
| `Ctrl` + `Shift` + `[` | 짝 맞는 **괄호로 이동** |

`Ctrl` + `Shift` + `F`는 우클릭 → **Format** → **Format SQL**과 같다. 
선택 없이 누르면 동작이 **버전·설정**에 따라 다르다 — 안 되면 **포맷할 SQL을 드래그**한 뒤 실행.

### 5.2 실행·결과

| 단축키 | 동작 |
|--------|------|
| **`Ctrl` + `Enter`** | **현재 SQL 문** 실행 (가장 자주 씀) |
| `Alt` + `X` | **스크립트 전체** 실행 |
| `Ctrl` + `\` | SQL 문을 **새 탭**에서 실행 |
| `Ctrl` + `Alt` + `Shift` + `X` | 문장마다 **결과 탭 분리** 실행 |
| `Ctrl` + `T` | **결과 패널** 표시/숨김 |
| `Ctrl` + `Shift` + `T` | 결과 패널 **최대화** / 복원 |
| `Ctrl` + `Shift` + `\` | 결과 탭 닫기 |
| `Ctrl` + `Shift` + `E` | **실행 계획**(Explain) |
| `Alt` + `↑` / `Alt` + `↓` | 이전/다음 **쿼리**로 이동 |

### 5.3 탐색·기타

| 단축키 | 동작 |
|--------|------|
| `F4` | 네비게이터에서 고른 **DB 객체** 편집기 열기 |
| `Ctrl` + `Shift` + `O` | **서버 출력** 콘솔 |
| `Ctrl` + `Shift` + `.` | 네비게이터 선택 연결을 **활성 연결**로 |
| `Ctrl` + `Alt` + `Shift` + `O` | 파일에서 SQL **불러오기** |
| `Ctrl` + `F2` | 현재 SQL 스크립트 **이름 변경** |

[[VS Code 사용법]]·[[Eclipse 사용법]]과 겹치는 키(`Ctrl`+`Shift`+`F` = 포맷)가 있어, **에디터를 오갈 때** 헷갈릴 수 있다.
...
3. 실행: §5.2 — **`Ctrl` + `Enter`**(현재 문), **`Alt` + `X`**(스크립트 전체) 
...
## 8. Auto-commit 해제 (Manual commit)
...
항상 끄려면 §8.2처럼 **연결 설정에 저장**한다.

### 8.2 연결마다 기본값을 Manual로 (권장·영구)
...
`Default`로 두면 **Connection type**(개발/테스트/운영)의 기본값을 따른다 → §8.3.
...
### 8.3 Connection type으로 운영만 기본 Manual
...
### 8.5 안 꺼지거나 다시 Auto로 돌아갈 때

1. 연결 **Initialization**에 Manual이 저장됐는지 확인 (§8.2) 
...
| SQL 포맷 | **`Ctrl` + `Shift` + `F`** (§5.1) 또는 우클릭 Format |
| 다크 테마 | Appearance |
| **Auto-commit 해제** | §8 (연결 Initialization / Connection type) |
...
| Auto-commit이 다시 켜짐 | §8.5 |
| 포맷 단축키 안 됨 | SQL **선택** 후 `Ctrl`+`Shift`+`F` · Keys 설정 확인 |
...
- [ ] CE 설치·워크스페이스 경로 
- [ ] **`Ctrl` + `Shift` + `F`**(포맷)·**`Ctrl` + `Enter`**(실행) 익히기 (§5) 
